...
const smartTicketSchema = z.object({
  ...
  seminarUrgency: z.boolean().default(false),
});
      ...
      <AnimatePresence>
        {isFocused && isOnline && (
          <motion.div 
            ...
            className="absolute -inset-10 bg-background/60 backdrop-blur-[2px] z-[5] pointer-events-none rounded-[3rem]"
          />
        )}
      </AnimatePresence>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 relative z-10 bento-card p-6 md:p-8">
        
        {/* Toggle Séminaire (Premium Priority) */}
        <div className="flex items-center justify-between p-4 rounded-2xl bg-muted/30 border border-red-500/20 group hover:border-red-500/30 transition-colors">
          <div className="flex flex-col">
            <Label className="flex items-center gap-2 text-red-400 font-bold tracking-widest uppercase text-[11px]">
              <AlertTriangle className="h-4 w-4" /> Urgence Séminaire
            </Label>
            <span className="text-xs text-muted-foreground mt-1">
              Activez uniquement si vous animez une formation OMEGA en cours.
            </span>
          </div>
          <Switch 
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="space-y-2" onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)}>
            ...
            <Input 
              ...
              {...register("title")} 
              className="bg-background/50 border-border text-foreground focus:border-blue-500 rounded-xl h-12 transition-all duration-300"
            />
          ...
          <div className="space-y-2">
            ...
            >
              <SelectTrigger className={`bg-background/50 text-foreground border-border rounded-xl h-12 transition-all duration-300 ${suggestedCategory ? 'border-green-500/50' : ''}`}>
        ...
        <div className="space-y-2" onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)}>
          ...
          <Textarea 
            ...
            className="bg-background/50 border-border text-foreground focus:border-blue-500 rounded-xl min-h-[140px] resize-none transition-all duration-300"
          />
        ...
        <AnimatePresence>
          {deflectionKey && (
            ...
            >
              <div className="p-5 mt-2 rounded-2xl bg-blue-900/20 border border-blue-500/30 backdrop-blur-xl flex flex-col gap-3">
                <div className="flex items-center gap-2 text-blue-500 font-bold">
                  <Lightbulb className="h-5 w-5" />
                  <span>💡 Solution Rapide Suggérée</span>
                </div>
                <h4 className="text-foreground font-semibold text-sm">{DEFLECTION_SOLUTIONS[deflectionKey].title}</h4>
                <ul className="space-y-2">
                  {DEFLECTION_SOLUTIONS[deflectionKey].steps.map((step, idx) => (
                    <li key={idx} className="flex items-start gap-2 text-sm text-blue-100/80">
        ...
        <div className="space-y-2" onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)}>
          <Label className="text-[10px] font-bold uppercase text-muted-foreground tracking-widest">Impact Métier (Criticité)</Label>
          <Select onValueChange={(val) => setValue("priority", (val || "low") as any)} defaultValue="low">
            <SelectTrigger className="bg-background/50 border-border text-foreground rounded-xl h-12 transition-all duration-300">
        ...
        <div className="pt-6 flex justify-end gap-3 mt-6">
          <Button 
            ...
            className="group neo-button w-full sm:w-auto bg-primary hover:opacity-90 text-primary-foreground rounded-xl font-bold px-8 h-12 shadow-md transition-all border-none cursor-pointer"
          >
            {isSubmitting ? (
              ...
              <span className="flex items-center"><Send className="mr-2 h-4 w-4 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" /> Émettre la requête</span>
            )}
          </Button>
        </div>
      </form>
